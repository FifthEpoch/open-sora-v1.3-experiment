...
- v1.3 naive experiment metrics:
    Open-Sora-1.3/naive_experiment/scripts/results/<run>/metrics.json
...
def load_v13_naive_metrics(metrics_json: Path) -> Tuple[List[dict[str, Any]], Dict[str, float]]:
    """
    Returns:
      - per_video_rows: one row per video, with baseline_* and finetuned_* columns (psnr/ssim/lpips + timing)
...
def load_v20_eval_dir(eval_dir: Path) -> Tuple[List[dict[str, Any]], Dict[str, Any]]:
    ...
    Returns:
      - detailed_metrics_rows (may be empty if not found)
...
def plot_metric_boxplots(
    out_path: Path,
    rows: list[dict[str, Any]],
    value_cols: Tuple[str, ...],  # e.g. ("psnr","ssim","lpips")
    title: str,
):
    """
    rows format:
...
def main():
    ...
    p.add_argument(
        "--only-baseline",
        action="store_true",
        help="If set, only compare baseline metrics (v1.3 baseline vs v2.0 baseline, if provided).",
    )
    args = p.parse_args()
    ...
    if not args.only_baseline:
        series[RunLabel("v1.3", "finetuned")] = {
    ...
    if args.v20_eval_dir:
        ...
        if v20_summary:
            ...
            if (not args.only_baseline) and ("lora_psnr_mean" in v20_summary):
    ...
    if v20_detailed:
        ...
        for r in v20_detailed:
            ...
            if (not args.only_baseline) and ("lora_psnr" in r):
                rows.append(
                    {

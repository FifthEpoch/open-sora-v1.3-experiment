# Configuration for baseline with LARGE TILES (less artifacts, less memory than no tiling)
# Key change: tile_size=16 (default, vs 4 in current configs) to reduce artifacts
# This should give MUCH better quality than tile_size=4 without requiring 128GB RAM

num_frames = 49  # Total frames (22 conditioning + 27 continuation)
condition_frame_length = 7  # More conditioning context (~27 pixel frames)
resolution = "480p"
aspect_ratio = "3:4"  # LANDSCAPE (H:W = 3:4 = 0.75) to match UCF-101's landscape format
fps = 24
frame_interval = 1

save_dir = "naive_experiment/results/baselines_largetile"
...
vae = dict(
    ...
    micro_frame_size=17,
    use_tiled_conv3d=True,  # Keep tiling enabled for memory efficiency
    tile_size=16,  # INCREASED from 4 to 16 (default) - should dramatically reduce artifacts
    normalization="video",
